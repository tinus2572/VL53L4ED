...
use crate::{consts, utils, BusOperation, Ranging, Initialized, DelayNs, Error, OutputPin, ResultsData, Vl53l4ed};
...
impl<B: BusOperation, XST: OutputPin, T: DelayNs> Vl53l4ed<Initialized, B, XST, T> {
    ...
    pub fn calibration_offset(mut self, target_dist: i16, nb_samples: i16) -> Result<(i16, Vl53l4ed<Initialized, B, XST, T>), Error<B::Error>> {
        ...
        let mut s: Vl53l4ed<Ranging, B, XST, T> = self.start_ranging()?;
        for _ in 0..10 {
            while !s.check_data_ready()? {}
            s.get_ranging_data()?;
            s.clear_interrupt()?;
        }
        let s: Vl53l4ed<Initialized, B, XST, T> = s.stop_ranging()?;

        // Device ranging
        let mut s: Vl53l4ed<crate::Ranging, B, XST, T> = s.start_ranging()?;
        for _ in 0..nb_samples {
            while !s.check_data_ready()? {}
            results = s.get_ranging_data()?;
            s.clear_interrupt()?;
            avg_dist += results.distance_mm as i16;
        }
        let mut s: Vl53l4ed<Initialized, B, XST, T> = s.stop_ranging()?;
        avg_dist /= nb_samples;
        measured_offset = target_dist - avg_dist;
        s.write_u16(VL53L4ED_RANGE_OFFSET_MM, (measured_offset * 4) as u16)?;

        Ok((measured_offset, s))
    }
    ...
    pub fn calibration_xtalk(mut self, target_dist: i16, nb_samples: i16) -> Result<(i16, Vl53l4ed<Initialized, B, XST, T>), Error<B::Error>> {
        let mut results: ResultsData;
        ...
        let s = s.stop_ranging()?;

        // Device ranging
        let mut s = s.start_ranging()?;
        ...
        s.write_u16(VL53L4ED_XTALK_PLANE_OFFSET_KCPS, (tmp_xtalk * 512.0) as u16)?;
