...
use vl53l4ed::{
    consts::{
        VL53L4ED_DEFAULT_I2C_ADDRESS, 
        VL53L4ED_NB_THRESHOLDS, 
        VL53L4ED_LAST_THRESHOLD},
...
fn write_results(tx: &mut Tx<USART2>, results: &ResultsData, width: usize) {
    ...
    for j in 0..width {
        ...
        #[cfg(not(any(feature="VL53L4ED_DISABLE_DISTANCE_MM", feature="VL53L4ED_DISABLE_TARGET_STATUS")))]
        {
            for i in 0..width {
                write!(
                    tx, 
        ...
        #[cfg(not(any(feature="VL53L4ED_DISABLE_SIGNAL_PER_SPAD", feature="VL53L4ED_DISABLE_AMBIENT_PER_SPAD")))]
        {
            for i in 0..width {
...
fn main() -> ! {
    ...
    let xshut_pin: Pin<'B', 4, Output> = gpiob.pb4.into_push_pull_output_in_state(High);
    ...
    let mut sensor = Vl53l4ed::new_i2c(
    ...
    let mut thresholds: [DetectionThresholds; VL53L4ED_NB_THRESHOLDS] = [DetectionThresholds::new(); VL53L4ED_NB_THRESHOLDS];
    ...
    thresholds[resolution as usize].zone_num |= VL53L4ED_LAST_THRESHOLD;
